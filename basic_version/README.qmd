...
## 3. The main route: the form → query params → filter cycle
...
The `parent_report_df` step works on the *filtered* `sample_df`. For each
distinct `parent_report` value, it computes a row count (`pl.len()`) and the
sums of the numeric columns. `.alias("report_count")` names the count column;
the summed columns keep their original names. The `report_count` is what the
template later uses to decide whether a row gets a plus button.
`.to_dicts()` then turns the DataFrame into a list of dicts, one per row —
the shape Jinja2 is happiest looping over.

### Building the chart payload

Chart.js (the JavaScript charting library used in the browser) wants its input
in a specific shape: a list of x-axis `labels`, and a list of `datasets`, each
with a `label` (legend text), `data` (list of y-values), and styling like
`borderColor`. The `line_graph` dict is exactly that shape, built from plain
Python dicts/lists, which serialize cleanly to JSON later.

Bits of syntax in there:

- `sorted_ts_df.to_dicts()` turns the DataFrame into a list of dicts, one per
  row — handy for looping.
- The `"labels": [...]` line is a *list comprehension*: build a list by
  evaluating the expression once per row.
...
### Handing it all to Jinja2

`templates.TemplateResponse(request, "index.html", {...})` renders the
template. The dict is the **context**: every key becomes a variable the
template can use. Because the data shaping happened just above, the context is
just plain variables (`line_graph`, `parent_report_rows`, ...) — Python
computes values, the template arranges them into HTML. That's the whole
Jinja2 contract.
...
- `parent_report_rows` came from `parent_report_df.to_dicts()`, so each `row`
  is a dict. Jinja2 lets you write `row.parent_report` or `row[column]`
  interchangeably for dict access.
...
## 5. The expandable rows: JS → FastAPI → HTML fragment
